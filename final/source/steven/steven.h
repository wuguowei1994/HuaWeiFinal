...
#define TIME_OUT 			9000000						//超时时间=9000ms;9s内无法找出最优解,判定case为无解
#define False 				0
#define True  				1
...
// void *memset(void *s,int ch, unsigned n):将s所指向的某一块内存中的每个字节的内容全部设置为ch指定的ASCII值, 块的大小由第三个参数指定
...
//example: #define exampleNum(n) num##n  use: int num=exampleNum(9); 将会扩展成 int num=num9;(num9必须是已知量)
...
{
public:
	int num, dim;						// num:合并起点和终点后，必经点总数, 如果是atsp问题有dim = 2*num, tsp问题有dim=num
	...
	void init(Road *roadp);				// O(n^2) 将atsp转换成tsp
...
public:
	static Graph *Gp;					//Graph的地址
	static int s,t;						//source,destination
	static int sLKHuseCn;				//寻路总共使用LKH的次数
	
	static long long roadCostSum[3];	//路径和,[0][1]是最新和上一次
	static long long roadCostSumMin;	//路径和的最优值(保证reIdCn最小的情况下)
    ...
    string fileName;  //result.csv的路径
	
	//for tsp
	VVNode Node[MaxV2];					//必经点的集合
	VVNode *firstActive,*lastActive;	//指向第一个active node,指向最后一个active node
	int num,numInit,numRepeat,numBak;	//num:图中点的总数;环路中当前必经点的数量,和初始化必经点数量,tsp每次的重复点数,加入重复点时备份变量
